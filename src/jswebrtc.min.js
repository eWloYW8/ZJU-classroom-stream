(function (global, factory) {
  ...
  var JSWebrtc = {
    ...
    ParseUrl: function(rtmp_url) {
        ...
        a.href = rtmp_url.replace("rtmp://", "http://").replace("webrtc://", window.location.protocol + "//").replace("rtc://", window.location.protocol + "//");
        ...
        if (window.location.protocol.search("https") == 0) {
            rtmp_url = rtmp_url.replace("http://", "https://");
        }
        var ret = {
            url: rtmp_url,
            schema: schema,
...
JSWebrtc.Player = function() {
    ...
    var Player = function(url, options) {
        ...
        // if (!url.match(/^webrtc?:\/\//) or  ) {
        //     throw "JSWebrtc just work with webrtc"
        // }
        if (!this.options.video) {
            throw "VideoElement is null"
        }
        this.proxy = this.options.proxy || "https://mcloudpush.cmc.zju.edu.cn:10443/player";
    ...
    Player.prototype.startLoading = function() {
        ...
        _self.pc.addEventListener("connectionstatechange",()=>{
            console.log(`pull status ${_self.pc.connectionState}`)

            if(_self.pc.connectionState === 'failed') {
                // 重新拉流
                _self.startLoading()
            }
        })
        ...
        // setInterval(() => {
        //     if (_self.pc) {
        //         _self.pc.getStats().then(stats => {
        //             stats.forEach(report => {
        //                 // 检查报告类型
        //                 if (report.type === 'inbound-rtp' && report.mediaType === 'video') {
        //                     // 提取相关指标
        //                     // const latency = report.roundTripTime;
        //                     // const packetLoss = report.packetsLost;
        //                     // const jitter = report.jitter;
                    
        //                     // 打印指标值
        //                     console.log('report----------',this.urlParams.stream, report);
        ...
        _self.pc.ontrack = function(event) {
            // _self.options.video.addEventListener("timeupdate", function() {
            //     //播放中断 事件处理
            //     var now = _self.options.video.currentTime;
            //     setTimeout(() => {
            //         if (now < _self.options.video.currentTime) {
            //             console.log("正在播放");
            //         } else {
            //             if (_self.options.video.currentTime == 0) {
            //                 console.log("播放未开始等待推流");
            //             } else {
            //                 console.log("播放中断重新拉流");
            //                 _self.pc.close();
            //                 _self.startLoading();
            //             }
            //         }
            ...
            // _self.pc.getSenders().forEach((sender) => {
            //     console.log(sender.getParameters('video'));
            // })
        };
        ...
        _self.pc.createOffer().then(function(offer) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                // }
                console.log(_self.urlParams.url);
                var data = {
                    url: _self.urlParams.url,
                    clientip: null,
                    sdp: offer.sdp
                };
                // console.log("offer: " + JSON.stringify(data));
                console.log("--------------------", _self.proxy);
                JSWebrtc.HttpPost(_self.proxy, JSON.stringify(data)).then(function(res) {
            ...
            // answer = answer.replace("H264", "VP9");
            console.log(answer);
            ...
            console.log('throw reason error:', reason)
            throw reason
        });
        if (this.autoplay) {
            this.play()
    ...
    Player.prototype.changeVideo = function(video) {
        this.options.video = video;
        console.log("切换后的video", this.options);
    ...
    Player.prototype.update = function() {
        ...
        if (!this.isPlaying) {
            ...
            let videoPlay = this.options.video.play()
            videoPlay.then().catch((error) =>{
                console.error(error)
